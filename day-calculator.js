function calculateDayInYear(date) {
  ...
  function validMonth(month) {
    return month && month >= 1 && month <= 12;//changed less than to less than or equals
  }

  function validDay(month, day) {
    return day && day >= 1 && day <= DAYS_IN_MONTH[month - 1];//changed < to <=
  }

  function calculateDayNumber(month, day) {
    var dayOfYear = day;//changed dayOfYear starting value from 1 to day
